...
const Appointment = () => {
    ...
    useEffect(() => {
        ...
        const bookAppointment = async () => {
            ...
            const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || "http://localhost:8080/"}appointment/new`, {
                method: "POST",
                headers: {
    ...
    useEffect(() => {
        ...
        const getAll = async () => {
            ...
            const res = await fetch(`${process.env.REACT_APP_API_BASE_URL || "http://localhost:8080/"}user/doctorDetails/${location.search.split("=")[1]}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer : ${authCtx.token}`
